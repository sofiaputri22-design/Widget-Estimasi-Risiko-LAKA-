...
def load_and_prepare_all():
    ...
    # Preprocessing numerik yang lebih kuat
    df['kecepatan'] = pd.to_numeric(df['kecepatan'], errors='coerce')
    df['kecepatan'] = df['kecepatan'].fillna(df['kecepatan'].median() if not df['kecepatan'].isna().all() else 40).astype(int)
    ...
    counts = df['jenis luka'].value_counts()
    df = df[df['jenis luka'].isin(counts[counts > 1].index)]
...
st.markdown("<div style='background: linear-gradient(135deg, #f1c40f 0%, #f39c12 100%); padding: 25px; border-radius: 15px; text-align: center; color: #2c3e50;'><h1 style='margin:0;'>Accident Risk Predictor</h1><p style='font-weight: bold; font-size: 1.2rem;'>Estimasi Tingkat Keparahan Kecelakaan Kabupaten Gresik</p></div>", unsafe_allow_html=True)
...
for col in features_list:
    if df_raw[col].dtype == 'object':
        ...
        user_inputs[col] = st.sidebar.selectbox(col.replace('_',' ').title(), opsi)
    else:
        # Handling error min/max/median dengan konversi eksplisit ke float lalu int
        val_min = int(float(df_raw[col].min()))
        val_max = int(float(df_raw[col].max()))
        val_med = int(float(df_raw[col].median()))
        user_inputs[col] = st.sidebar.slider(col.title(), val_min, val_max, val_med)
...
if st.button("MULAI ANALISIS RISIKO", use_container_width=True):
    ...
    with col2:
        ...
        ax.pie(p_final, labels=labels, autopct='%1.1f%%', colors=slice_colors, startangle=140, shadow=True, textprops={'fontweight':'bold', 'fontsize': 12})
        st.pyplot(fig)