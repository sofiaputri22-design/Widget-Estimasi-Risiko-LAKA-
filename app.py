...
def load_and_prepare_all():
    ...
    # Membersihkan baris jika target 'jenis luka' kosong
    df = df.dropna(subset=['jenis luka'])

    # PROTEKSI UTAMA: Hapus kategori yang hanya memiliki 1 sampel agar stratify tidak error
    ...
    # Split data dengan stratify (Sekarang aman karena sampel minimal > 1)
...
for col in features_list:
    if df_raw[col].dtype == 'object':
        ...
        user_inputs[col] = st.sidebar.selectbox(f\"{col.replace('_',' ').title()}\", opsi)
    else:
